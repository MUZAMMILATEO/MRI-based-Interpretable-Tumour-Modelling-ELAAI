...
## 🧩 Methodology (left) & Architectures (right)
...
## 📊 Results: Bladder Segmentation Comparisons
...
## 🎯 Results: Tumour Delineation Generalisation
...
## 🔍 Results: Tumour Likelihood Maps

The figure below presents tumour likelihood maps generated by **SLIP-Net** and other state-of-the-art frameworks, overlaid on MRI scans. Notably, these maps were produced even though SLIP-Net was trained **only on normal bladder data**, showcasing the framework’s potential for **out-of-distribution generalisation**.
...
Each module has its own dependencies. Please set up the environment according to the module you want to use:
- **MFA-Net**  
  ```bash
  cd MFA_Net
